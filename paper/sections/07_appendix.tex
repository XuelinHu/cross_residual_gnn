...
\end{equation}
where $\beta \ge 0$ controls the strength of the reused hidden state. In this study, we set $\beta=1$.
...
\section{Targeted Ablation Tables}
\label{app:targeted_ablations}

This section collects the confirmatory five-fold ablations that support the main mechanism claims in the paper. The tables below separate the graph-level gate study, the cross-gate study on cross-favored targets, and the residual-routing summary used in the main text.

\input{../md/peerj_targeted_ablation_appendix.tex}

\section{Exploratory Sensitivity Tables}
\label{app:sensitivity}

This appendix section is reserved for the older single-fold sensitivity analyses over depth, dropout, and learning rate on PROTEINS, DD, and ENZYMES. These tables and figures should be explicitly labeled as exploratory, because they are useful for optimization interpretation but do not have the same evidential status as the five-fold \texttt{V3} benchmark and targeted ablations.
