...
The four external baselines are configured as follows. GraphSAGEBaseline stacks SAGEConv layers with mean aggregation. GINBaseline stacks GINConv layers, each containing a two-layer MLP with ReLU activation. JKNetBaseline stacks GCNConv layers and aggregates representations from all depths via Jumping Knowledge with concatenation mode (\texttt{cat}). APPNPBaseline uses a GCNConv backbone followed by APPNP propagation with $K=10$ steps and teleport probability $\alpha=0.1$.

All experiments are run under a single version-controlled workflow. Each fold's best checkpoint is selected by minimum validation loss, and the corresponding test accuracy is recorded as the fold result. The five fold-level test accuracies are then summarized as mean $\pm$ population standard deviation.
