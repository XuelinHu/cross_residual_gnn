...
def build_jobs(version: str, folds: Sequence[int]) -> List[Tuple[str, List[str]]]:
    ...
    for target in TARGETS:
        ...
        for setting_name, setting in PARAM_SETTINGS:
            for fold in folds:
                config = {**target, **COMMON_ARGS, **setting}
                config["fold"] = fold
                config["exp_tag"] = (
                    f"residual_param_{target['ds'].lower()}_{target['gname'].lower()}_"
                    f"{target['name'].lower()}_{setting_name}"
                )
                jobs.append((f"{slug}_{setting_name}_fold{fold}", build_command(config, version)))
    return jobs


def alias_patterns(ds: str, gname: str, name: str, fold: str, exp_tag: str) -> List[str]:
    suffix = exp_tag.split(f"{name.lower()}_", 1)[-1]
    aliases = [f"train_{ds}_{gname}_{name}_fold{fold}_{exp_tag}__*.json"]
    if suffix.startswith("topk_") or suffix.startswith("sparse_"):
        aliases.append(
            f"train_{ds}_{gname}_{name}_fold{fold}_residual_mode_{ds.lower()}_{gname.lower()}_{name.lower()}_{suffix}__*.json"
        )
    return aliases


def result_exists(version: str, cmd: Sequence[str]) -> bool:
    ...
    for pattern in alias_patterns(ds, gname, name, fold, exp_tag):
        if any(version_logs.glob(pattern)):
            return True
    return False


def filter_missing_jobs(version: str, jobs: Iterable[Tuple[str, List[str]]]) -> List[Tuple[str, List[str]]]:
    return [job for job in jobs if not result_exists(version, job[1])]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run 5-fold topk_ratio / sparse_lambda sweeps.")
