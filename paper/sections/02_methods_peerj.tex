...
All reported runs follow the same latest versioned experimental workflow. The recommended workflow entry is \texttt{py/run\_final\_v3\_pipeline.py}, and the associated outputs are stored in \texttt{logs/LATEST}, \texttt{records/LATEST}, and \texttt{runs/LATEST}. Preprocessing is intentionally light, and the final protocol does not add task-specific feature engineering, graph augmentation, or custom edge redesign. This common workflow is important for reliability: every main benchmark and targeted ablation result reported in the paper is computed under the same split logic, selection rule, and logging structure.
...
Targeted ablations are organized around three mechanism questions. First, gate-control ablations test whether adaptive residual injection is stronger or more stable than fixed-strength reuse on a strong graph-level residual target. Second, cross-gate ablations examine settings where a cross architecture is already locally competitive, asking whether adaptive gating remains robust in those cases. Third, residual-routing ablations compare learnable, top-k, and sparse routing modes on representative residual- and cross-oriented targets, so that the study can move beyond architecture labels and evaluate how residual information is injected.
