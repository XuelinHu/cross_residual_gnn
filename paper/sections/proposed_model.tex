...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/cr_gnn_family_overview.pdf}
\caption{Overview of the CR-GNN family used in this paper. The main comparison moves from plain stacking to node-level residual reuse, node-level cross exchange, graph-level residual refinement, and graph-level cross-summary exchange.}
\label{fig:crgnn_family}
\end{figure}
...
The full implementation family contains six architectures, including the auxiliary GraphCondGNN variant. The main paper experiments focus on five core models, but all family members differ only in how they update $\mathbf{H}^{(\ell)}$ and how they reuse graph-level embeddings across blocks.
...
\end{algorithmic}
\end{minipage}
\caption{Pseudocode of the shared CR-GNN forward procedure}
