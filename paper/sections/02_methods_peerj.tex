...
\end{equation}
Here $\Phi_{\mathrm{in}}:\mathbb{R}^{n\times d_{\mathrm{in}}}\rightarrow\mathbb{R}^{n\times d}$ denotes the branch-specific input operator, $\mathrm{Pool}(\cdot)$ is global mean pooling over nodes, and
\begin{equation}
...
\mathcal{R}(\mathbf{U}) =
\begin{cases}
\mathbf{U}, & \text{learnable dense routing}, \\
\mathbf{U}\odot \mathbf{M}_{k}, & \text{top-k routing}, \\
\mathrm{softshrink}_{\lambda}(\mathbf{U}), & \text{sparse routing},
\end{cases}
\end{equation}
where $\mathbf{M}_{k}$ keeps the largest-magnitude channels along the feature dimension and $\lambda$ is the sparse-routing threshold.
...
\textbf{NodeResGNN.} The first residual variant stores the previous hidden state within the same branch. To match the implementation, the first residual input is zero:
\begin{equation}
...
\end{equation}
with zero-initialized exchanged states
\begin{equation}
\mathbf{P}_{1}^{(0)}=\mathbf{0}, \qquad \mathbf{P}_{2}^{(0)}=\mathbf{0}, \qquad
\mathbf{P}_{b}^{(\ell+1)}=\mathbf{H}_{b}^{(\ell)}.
\end{equation}
Each branch receives the previous state of the other branch:
\begin{align}
...
\textbf{GraphResGNN.} This variant chains three graph-conditioned blocks. Let $\mathcal{B}(\mathbf{X},\mathbf{A},\mathbf{g})$ denote one graph-conditioned block. Inside the block, a graph-level state $\mathbf{g}\in\mathbb{R}^{B\times d}$ is broadcast back to nodes by batch indexing and added before each hidden message-passing layer:
\begin{align}
...
\end{align}
If no previous graph state is available, the block uses $\mathbf{g}=\mathbf{0}$. GraphResGNN then applies the recurrence
\begin{equation}
\mathbf{g}^{(0)}=\mathbf{0}, \qquad
\mathbf{g}^{(t)} = \mathcal{B}_{t}\!\left(\mathbf{X}, \mathbf{A}, \mathbf{g}^{(t-1)}\right), \qquad t=1,2,3,
\end{equation}
and uses $\mathbf{g}^{(3)}$ for classification.

\textbf{GraphCrossGNN.} Four blocks are organized as two sequential pairs. In this model, each block uses graph-level exchange only at the pooled embedding stage, not inside node updates. Let $\mathcal{C}(\mathbf{X},\mathbf{A},\mathbf{g})$ denote such a block:
\begin{align}
\mathbf{Z}^{(0)} &= \psi\!\left(\Phi_{\mathrm{in}}(\mathbf{X},\mathbf{A})\right), \\
...
\end{align}
With two branch states initialized as $\mathbf{g}^{(0)}_{1}=\mathbf{g}^{(0)}_{2}=\mathbf{0}$, the stage-wise recurrence is
\begin{align}
...
\begin{enumerate}
    ...
    \item \textbf{Parameter sweeps.} Residual-parameter sweeps vary top-k ratios and sparsity strengths on the same representative targets.
\end{enumerate}
...
The primary metric is graph classification accuracy on the held-out test fold. Main benchmark and targeted ablation results are reported as five-fold mean accuracy with standard deviation. Five-fold experiments provide confirmatory evidence; parameter sweeps are reported as supporting analyses only.
