\begin{table*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{四个代表性目标上的残差路由与参数扫描摘要（转置形式）。数值为五折平均准确率 $\pm$ 标准差。加粗表示该列最佳。}
\label{tab:residual_routing_full}
\begin{tabular}{lcccc}
\toprule
路由方式 & AIDS+GraphRes+GIN & PROT+GraphRes+SAGE & AIDS+NodeCross+GAT & DD+NodeCross+GAT \\
\midrule
Learnable & \textbf{0.9500 $\pm$ 0.0087} & 0.7178 $\pm$ 0.0411 & 0.8970 $\pm$ 0.0491 & 0.7080 $\pm$ 0.0172 \\
Top-k 0.25 & 0.9075 $\pm$ 0.0237 & 0.7241 $\pm$ 0.0407 & 0.9160 $\pm$ 0.0128 & 0.7062 $\pm$ 0.0249 \\
Top-k 0.50 & 0.9330 $\pm$ 0.0251 & 0.7151 $\pm$ 0.0397 & 0.8960 $\pm$ 0.0483 & 0.7029 $\pm$ 0.0160 \\
Top-k 0.75 & 0.9240 $\pm$ 0.0287 & 0.7214 $\pm$ 0.0318 & 0.8960 $\pm$ 0.0489 & 0.7164 $\pm$ 0.0248 \\
Sparse 0.02 & 0.9470 $\pm$ 0.0181 & 0.7160 $\pm$ 0.0248 & 0.9060 $\pm$ 0.0268 & 0.7147 $\pm$ 0.0259 \\
Sparse 0.05 & 0.9250 $\pm$ 0.0152 & \textbf{0.7259 $\pm$ 0.0475} & \textbf{0.9180 $\pm$ 0.0162} & \textbf{0.7181 $\pm$ 0.0196} \\
Sparse 0.10 & 0.8315 $\pm$ 0.0549 & 0.7088 $\pm$ 0.0521 & 0.9025 $\pm$ 0.0139 & 0.7105 $\pm$ 0.0243 \\
\midrule
最佳 & Learnable & Sparse 0.05 & Sparse 0.05 & Sparse 0.05 \\
\bottomrule
\end{tabular}
\end{table*}

\begin{table}[t]
\centering
\footnotesize
\caption{不同架构家族上的路由优胜摘要。}
\label{tab:residual_routing_family}
\begin{tabular}{lcccc}
\toprule
目标 & 最佳 learnable & 最佳 top-k & 最佳 sparse & 总体最优 \\
\midrule
AIDS + GraphRes + GIN & 0.9500 & 0.9330 & 0.9470 & Learnable \\
PROTEINS + GraphRes + SAGE & 0.7178 & 0.7241 & 0.7259 & Sparse \\
AIDS + NodeCross + GAT & 0.8970 & 0.9160 & 0.9180 & Sparse \\
DD + NodeCross + GAT & 0.7080 & 0.7164 & 0.7181 & Sparse \\
\bottomrule
\end{tabular}
\end{table}
