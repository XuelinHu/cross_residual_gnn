...
We compare the five architectures under the same GCN-style message-passing backbone, the same dataset protocol, and the same validation-based model-selection rule. All reported runs follow the same reproducible evaluation policy. The important point for the paper is that every architecture is tested under the same data protocol, the same operator backbone, and the same model-selection criterion, so the comparison isolates the effect of information-flow design rather than changes in optimization or preprocessing.
